    ...
    <div v-if="filteredBoardList.length > 0" class="board-list">
      <div v-for="(board, index) in filteredBoardList" :key="index" class="board-item" @click="goToBoardRead(board.dealId)">        
        <div class="board-item-image">         
          <img :src="board.imageFileListDTO[0] ? API_URLS.BOARD + '/api/board/read/download/' + board.imageFileListDTO[0].imgId : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTKC5SbJx2Wf9ewguE1uvgE3zc5TRnX1XfOsA&s'" alt="Book Image" />
        </div>
